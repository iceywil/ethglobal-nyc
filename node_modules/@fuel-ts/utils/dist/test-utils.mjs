var __defProp = Object.defineProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });

// src/test-utils/getForcProject.ts
import { existsSync, readFileSync } from "fs";
import { join } from "path";

// src/utils/arrayify.ts
import { FuelError, ErrorCode } from "@fuel-ts/errors";
var arrayify = /* @__PURE__ */ __name((value, name, copy = true) => {
  if (value instanceof Uint8Array) {
    if (copy) {
      return new Uint8Array(value);
    }
    return value;
  }
  if (typeof value === "string" && value.match(/^0x([0-9a-f][0-9a-f])*$/i)) {
    const result = new Uint8Array((value.length - 2) / 2);
    let offset = 2;
    for (let i = 0; i < result.length; i++) {
      result[i] = parseInt(value.substring(offset, offset + 2), 16);
      offset += 2;
    }
    return result;
  }
  const nameMessage = name ? ` ${name} -` : "";
  const message = `invalid data:${nameMessage} ${value}
If you are attempting to transform a hex value, please make sure it is being passed as a string and wrapped in quotes.`;
  throw new FuelError(ErrorCode.INVALID_DATA, message);
}, "arrayify");

// src/utils/hexlify.ts
var HexCharacters = "0123456789abcdef";
function hexlify(data) {
  const bytes = arrayify(data);
  let result = "0x";
  for (let i = 0; i < bytes.length; i++) {
    const v = bytes[i];
    result += HexCharacters[(v & 240) >> 4] + HexCharacters[v & 15];
  }
  return result;
}
__name(hexlify, "hexlify");

// src/utils/normalizeString.ts
import { ErrorCode as ErrorCode2, FuelError as FuelError2 } from "@fuel-ts/errors";
var normalizeString = /* @__PURE__ */ __name((str) => {
  const transformations = [
    (s) => s.replace(/\s+/g, "-"),
    // spaces to -
    (s) => s.replace(/\./g, "-"),
    // dots to -
    (s) => s.replace(/_/g, "-"),
    // underscore to -
    (s) => s.replace(/-[a-z]/g, (match) => match.slice(-1).toUpperCase()),
    // delete '-' and capitalize the letter after them
    (s) => s.replace(/-/g, ""),
    // delete any '-' left
    (s) => s.replace(/^\d+/, ""),
    // removes leading digits
    (s) => s[0].toUpperCase() + s.slice(1)
    // capitalize first letter
  ];
  const output = transformations.reduce((s, t) => t(s), str);
  if (output === "") {
    const errMsg = `The provided string '${str}' results in an empty output after`.concat(
      ` normalization, therefore, it can't normalize string.`
    );
    throw new FuelError2(ErrorCode2.PARSE_FAILED, errMsg);
  }
  return output;
}, "normalizeString");

// src/utils/date-time.ts
var TAI64_LEAP_SECONDS = 37;
var TAI64_UNIX_OFFSET = BigInt(2 ** 62) + BigInt(TAI64_LEAP_SECONDS);

// src/utils/sleep.ts
function sleep(time) {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(true);
    }, time);
  });
}
__name(sleep, "sleep");

// src/utils/base58.ts
import { ErrorCode as ErrorCode3, FuelError as FuelError3 } from "@fuel-ts/errors";
import { bn } from "@fuel-ts/math";
var BN_0 = bn(0);
var BN_58 = bn(58);

// src/utils/dataSlice.ts
import { ErrorCode as ErrorCode4, FuelError as FuelError4 } from "@fuel-ts/errors";

// src/utils/toUtf8Bytes.ts
import { FuelError as FuelError5, ErrorCode as ErrorCode5 } from "@fuel-ts/errors";

// src/utils/bytecode.ts
import { gzipSync, gunzipSync } from "fflate";

// src/test-utils/getForcProject.ts
var getProjectBuildDir = /* @__PURE__ */ __name((params) => join(params.projectDir, "out", params.build), "getProjectBuildDir");
var getProjectReleaseDir = /* @__PURE__ */ __name((params) => join(params.projectDir, "out", params.build), "getProjectReleaseDir");
var getProjectTempDir = /* @__PURE__ */ __name((params) => join(getProjectBuildDir(params), "__temp__"), "getProjectTempDir");
var getProjectAbiPath = /* @__PURE__ */ __name((params) => join(getProjectBuildDir(params), `${params.projectName}-abi.json`), "getProjectAbiPath");
var getProjectBinPath = /* @__PURE__ */ __name((params) => join(getProjectBuildDir(params), `${params.projectName}.bin`), "getProjectBinPath");
var getProjectStorageSlotsPath = /* @__PURE__ */ __name((params) => join(getProjectBuildDir(params), `${params.projectName}-storage_slots.json`), "getProjectStorageSlotsPath");
var getProjectAbiName = /* @__PURE__ */ __name((params) => `${params.projectName}-abi`, "getProjectAbiName");
var getProjectNormalizedName = /* @__PURE__ */ __name((params) => normalizeString(params.projectName), "getProjectNormalizedName");
var getProjectAbi = /* @__PURE__ */ __name((params) => {
  const projectPath = getProjectAbiPath(params);
  const abiContents = JSON.parse(readFileSync(projectPath, "utf-8"));
  return abiContents;
}, "getProjectAbi");
var getProjectStorageSlots = /* @__PURE__ */ __name((params) => {
  const storageSlotsFilePath = getProjectStorageSlotsPath(params);
  if (!existsSync(storageSlotsFilePath)) {
    return [];
  }
  const storageSlots = JSON.parse(readFileSync(storageSlotsFilePath, "utf-8"));
  return storageSlots;
}, "getProjectStorageSlots");
var getForcProject = /* @__PURE__ */ __name((params) => {
  const buildDir = getProjectBuildDir(params);
  const tempDir = getProjectTempDir(params);
  const binPath = getProjectBinPath(params);
  const binHexlified = hexlify(readFileSync(binPath));
  const abiPath = getProjectAbiPath(params);
  const abiName = getProjectAbiName(params);
  const abiContents = getProjectAbi(params);
  const normalizedName = getProjectNormalizedName(params);
  const storageSlots = getProjectStorageSlots(params);
  const inputGlobal = `${buildDir}/*-abi.json`;
  return {
    name: params.projectName,
    storageSlots,
    normalizedName,
    buildDir,
    tempDir,
    binPath,
    binHexlified,
    abiPath,
    abiName,
    abiContents,
    inputGlobal
  };
}, "getForcProject");

// src/test-utils/expectToBeInRange.ts
import { FuelError as FuelError6 } from "@fuel-ts/errors";
var expectToBeInRange = /* @__PURE__ */ __name((params) => {
  const { value, min, max } = params;
  if (value >= min && value <= max) {
    return true;
  }
  throw new FuelError6(
    FuelError6.CODES.INVALID_INPUT_PARAMETERS,
    `Expected value: '${value}' to be within range: '${min}-${max}'`
  );
}, "expectToBeInRange");

// src/test-utils/constants.ts
var ASSET_A = "0x0101010101010101010101010101010101010101010101010101010101010101";
var ASSET_B = "0x0202020202020202020202020202020202020202020202020202020202020202";

// src/test-utils/wait-until-unreachable.ts
async function waitUntilUnreachable(url) {
  let isLive;
  try {
    await fetch(url);
    isLive = true;
  } catch (e) {
    isLive = false;
  }
  if (!isLive) {
    return;
  }
  await sleep(250);
  await waitUntilUnreachable(url);
}
__name(waitUntilUnreachable, "waitUntilUnreachable");

// src/test-utils/vitest.matchers.ts
import { bn as bn2 } from "@fuel-ts/math";
var setupTestMatchers = /* @__PURE__ */ __name((expect) => {
  expect.extend({
    toEqualBn(received, expected) {
      const actualBn = bn2(received);
      const expectedBn = bn2(expected);
      const pass = actualBn.eq(expectedBn);
      if (pass) {
        return {
          pass,
          message: /* @__PURE__ */ __name(() => `Expected ${actualBn} not to equal ${expectedBn}`, "message"),
          actual: actualBn
        };
      }
      return {
        pass,
        message: /* @__PURE__ */ __name(() => `Expected ${actualBn} to equal ${expectedBn}`, "message"),
        actual: expectedBn
      };
    }
  });
}, "setupTestMatchers");
export {
  ASSET_A,
  ASSET_B,
  expectToBeInRange,
  getForcProject,
  getProjectAbi,
  getProjectAbiName,
  getProjectAbiPath,
  getProjectBinPath,
  getProjectBuildDir,
  getProjectNormalizedName,
  getProjectReleaseDir,
  getProjectStorageSlots,
  getProjectStorageSlotsPath,
  getProjectTempDir,
  setupTestMatchers,
  waitUntilUnreachable
};
//# sourceMappingURL=test-utils.mjs.map