...
var findPrograms = /* @__PURE__ */ __name((pathOrGlob, opts) => {
  ...
  return allTomlPaths.map((path2) => ({ path: path2, isWorkspace: readForcToml(path2).workspace !== void 0 })).filter(({ isWorkspace }) => !isWorkspace).map(({ path: path2 }) => ({ path: dirname(path2), swayType: readSwayType(dirname(path2)) })).filter(({ swayType }) => swayType !== "library" /* library */);
}, "findPrograms");
...
async function generateTypesForProgramType(config, paths, programType) {
  ...
  if (isScript || isPredicate) {
    const loaderFiles = paths.flatMap((dirpath) => {
      ...
      return globSync2(glob, { cwd }).map((filename) => `${dirpath}/out/${filename}`);
    });
...
var dev = /* @__PURE__ */ __name(async (config) => {
  ...
  const workspaceFilePaths = [contracts, predicates, scripts].flat().flatMap((dir) => [
    dir,
    globSync3(`${dir}/**/*.toml`, { cwd }),
    globSync3(`${dir}/**/*.sw`, { cwd })
  ]).flat();
  try {
...
import { existsSync as existsSync3, statSync, writeFileSync as writeFileSync5 } from "fs";
import { dirname as dirname2, join as join3, relative, resolve as resolve3 } from "path";
...
function init(program) {
  ...
  const convertFilePathToDir = /* @__PURE__ */ __name((filePath) => statSync(resolve3(path2, filePath)).isDirectory() ? filePath : dirname2(filePath), "convertFilePathToDir");
  const [contracts, scripts, predicates] = ["contracts", "scripts", "predicates"].map(
    (optionName) => {
      ...
      const programDirs = programs.filter(({ swayType }) => swayType === selectedSwayType).map(({ path: programPath }) => relative(path2, programPath));
