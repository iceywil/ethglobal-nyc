...
var findPrograms = /* @__PURE__ */ __name((pathOrGlob, opts) => {
  ...
  return allTomlPaths.map((path) => ({ path, isWorkspace: readForcToml(path).workspace !== void 0 })).filter(({ isWorkspace }) => !isWorkspace).map(({ path }) => ({ path: (0, import_path.dirname)(path), swayType: readSwayType((0, import_path.dirname)(path)) })).filter(({ swayType }) => swayType !== "library" /* library */);
}, "findPrograms");
...
// src/cli/commands/build/buildSwayProgram.ts
var buildSwayProgram = /* @__PURE__ */ __name(async (config, path) => {
...
var workspaceFileChanged = /* @__PURE__ */ __name((state) => async (_event, path) => {
  log(`
File changed: ${path}`);
  await buildAndDeploy(state.config);
}, "workspaceFileChanged");
var configFileChanged = /* @__PURE__ */ __name((state) => async (_event, path) => {
  log(`
File changed: ${path}`);
...
function init(program) {
  ...
  const convertFilePathToDir = /* @__PURE__ */ __name((filePath) => (0, import_fs8.statSync)((0, import_path5.resolve)(path, filePath)).isDirectory() ? filePath : (0, import_path5.dirname)(filePath), "convertFilePathToDir");
  const [contracts, scripts, predicates] = ["contracts", "scripts", "predicates"].map(
    (optionName) => {
      ...
      const programs = paths.map(convertFilePathToDir).flatMap((pathOrGlob) => findPrograms(pathOrGlob, { cwd: path }));
      const programDirs = programs.filter(({ swayType }) => swayType === selectedSwayType).map(({ path: programPath }) => (0, import_path5.relative)(path, programPath));
...
var configFileChanged2 = /* @__PURE__ */ __name((state) => async (_event, path) => {
  log(`
File changed: ${path}`);
